...
;(function ( $, window, document, undefined ) {
    ...
    $.extend(Plugin.prototype, {
        init: function () {
            ...
            // Loading first script
            $.getScript(src).done(function(){
                // When first is loaded loading another
                $.getScript(src2).done(function(){
                    // After that starting with plugin functions
        ...
        searchPlace: function (constructor, searchBox, map, markerTable, element, settings) {
            ...
            google.maps.event.addListener(searchBox, 'place_changed', function() {
                ...
                if(!place.geometry){
                    ...
                    geocoder.geocode({'address': address+ "," + settings.sidebar.searchBox.searchByCountry[1]}, function(results, status) {
