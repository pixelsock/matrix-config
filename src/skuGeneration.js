...
function getPrefix() {
  ...
  if (productLine.includes("Anti-Ligature")) return "A";
...
function generateSku(selectedOptions) {
  ...
  if (getPrefix() === "S") {
    ...
    if (
      selectedOptions.find(
        (option) => option.dataName === "Custom Size Checkbox",
      )
    ) {
      isCustomSize = true;
    }
    ...
    switch (mirrorControlsValue) {
      case "Wall Switch Only":
        if (accessoriesSku === "NLAF")
          accessoriesSku = "AN"; // Both Night-Light and Anti-Fog selected
        else if (accessoriesSku === "") accessoriesSku = "NA"; // No accessories selected
        break;
      case "Touch Sensor - Light Controls Only":
        if (accessoriesSku === "AF")
          accessoriesSku = "AT"; // Touch Sensor and Anti-Fog selected
        else if (accessoriesSku === "NL")
          accessoriesSku = "NT"; // Touch Sensor and Night-Light selected
        else if (accessoriesSku === "")
          accessoriesSku = "TS"; // No accessories selected
        else if (accessoriesSku === "NLAF") accessoriesSku = "AL"; // All accessories selected
        break;
      case "CCTSync":
        if (accessoriesSku === "AF")
          accessoriesSku = "CF"; // CCTSync and Anti-Fog selected
        else if (accessoriesSku === "NL")
          accessoriesSku = "CN"; // CCTSync and Night-Light selected
        else if (accessoriesSku === "")
          accessoriesSku = "CT"; // No accessories selected
    ...
    const frameColorOption = selectedOptions.find(
      (option) =>
  ...
  let isCustomSize = false;
  if (
    selectedOptions.find((option) => option.dataName === "Custom Size Checkbox")
  ) {
    isCustomSize = true;
  }
  ...
  selectedOptions.forEach((option) => {
    ...
    if (skuMapping[category] && skuMapping[category][value]) {
      ...
      if (
        category === "Custom Size Checkbox" &&
        value === "Custom-Size-Checkbox"
      ) {
        isCustomSize = true;
      }
  ...
  if (prefix === "D") {
    const frameThicknessOption = selectedOptions.find(
      (option) => option.dataName === "Frame Thickness",
    );
    if (frameThicknessOption) {
      const frameThickness =
        skuMapping["Frame Thickness"][frameThicknessOption.value];
      skuComponents["Mirror Style"] =
        frameThickness + skuComponents["Mirror Style"];
